...
| en-clip-mt (번역 파이프라인) | 0.520 | 0.680 | 0.740 | 0.860 | 0.578 | 0.623 |
| en-clip-mt + 하이브리드 필터 | 0.640 | 0.780 | 0.860 | 0.900 | 0.716 | 0.731 |

**en-clip-mt(질의 번역 → 영어 CLIP)가 m-clip을 크게 이기지만 ko-clip에는 못 미친다.**
번역 파이프라인은 이미지 인덱스가 m-clip과 동일(영어 CLIP ViT-B/32)한데도 hit@10이
48%→86%로 뛴다 — 다국어 "정렬"보다 명시적 "번역"이 커머스 어휘를 훨씬 잘 옮긴다는 뜻.
남은 실패 7건은 거의 전부 소형 번역기(opus-mt-ko-en)의 오역이다: "남성 운동화 → Male
Aggression", "여성 힐 → Female Hill", "여성 원피스 → The Women's Constance",
"남아용 신발 → Namaste shoes". 반대로 번역이 맞은 "여름용 원피스 → The summer dress"는
rr 1.0으로 1위에 꽂힌다. **번역 파이프라인의 품질 상한 = 번역기 품질**이며, 더 큰 MT를
쓰면 격차가 줄겠지만 질의당 번역 지연이 붙는다(오역 전수 기록은
`eval/results_en-clip-mt.json`의 `translation` 필드).
